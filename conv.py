                ...
                with tf.Session(graph=self.graph) as session:
                    ...
                    for step in range(num_steps):
                        ...
                        if (step % 100 == 0):
                            ...
                            if accuracy(val_preds, self.val_Y) > best_val_accuracy:
                                best_val_accuracy = accuracy(val_preds, self.val_Y)
                                best_train_accuracy = accuracy(train_preds, self.train_Y)
                                best_step = step

                            print('Best Validation accuracy: %.1f%%' % best_val_accuracy)
                            print('Best Full train accuracy: %.1f%%' % best_train_accuracy)
